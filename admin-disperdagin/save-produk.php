  ...
  $nama_gambar_baru = $gmbr; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {    
                move_uploaded_file($file_tmp, 'foto/produk/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO produk (nama_produk, kategori, alamat, gmbr) VALUES ('$nama_produk', '$kategori', '$alamat', '$nama_gambar_baru')";
                  ...
                  if(!$result){
                    ...
                    echo "<script>alert('Data Produk berhasil ditambah.');window.location='produk.php';</script>";
                  }
   ...
   $query = "INSERT INTO produk (nama_produk, kategori, alamat, gmbr) VALUES ('$nama_produk', '$kategori', '$alamat')";
                  $result = mysqli_query($koneksi, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($koneksi).
