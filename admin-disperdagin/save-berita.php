  ...
  $nama_gambar_baru = $foto; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {    
                move_uploaded_file($file_tmp, 'foto/berita/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO berita (judul_berita, kategori, isi, foto) VALUES ('$judul','$kategori','$isi','$foto')";
   ...
   $query = "INSERT INTO berita (judul, kategori, isi, foto) VALUES ('$judul','$kategori','$isi', '$foto')";
                  $result = mysqli_query($koneksi, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($koneksi).
