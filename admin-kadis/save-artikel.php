  ...
  $nama_gambar_baru = $gambar; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {    
                move_uploaded_file($file_tmp, 'artikel/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO artikel (id_artikel, judul, penulis, deskripsi, gambar) VALUES ('$id','$judul','$penulis','$deskripsi','$nama_gambar_baru')";
   ...
   $query = "INSERT INTO artikel (id_artikel, judul, penulis, deskripsi, gambar) VALUES ('$id','$judul','$penulis','$deskripsi')";
                  $result = mysqli_query($koneksi, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($koneksi).
